...
class PricingEngineImpl(discountRuleRepository: DiscountRuleRepository) extends PricingEngine with Logging {
  ...
  override def priceBasket(items: Set[Item]): IO[Either[List[Error], PriceBasketResult]] = IO {
    ...
    val discounts = discountRuleRepository.getAll
      .foldLeft(ListBuffer(): ListBuffer[Discount])((acc, dr) => {
